"""
多步学习 (N-step Learning) 实现

多步学习是一种通过计算n步回报来改进值函数估计的方法。
通过在计算TD目标时使用多个时间步的累积奖励，可以加速价值传播并减少估计偏差。

N步回报计算: R_t + γR_{t+1} + γ^2R_{t+2} + ... + γ^{n-1}R_{t+n-1} + γ^n V(s_{t+n})

参考文献: https://arxiv.org/abs/1602.01783 (Rainbow论文中的一个组件)
"""
...
class NStepReplayBuffer:
    ...
    def __init__(self, capacity, n_steps=3, gamma=0.99):
        self.buffer = deque(maxlen=capacity)  # 主缓冲区
        self.n_step_buffer = deque(maxlen=n_steps)  # n步经验临时缓冲区
        self.n_steps = n_steps  # 步数
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        # 如果n步缓冲区未满且当前状态不是终止状态，则不添加到主缓冲区
...
class NStepDQNAgent:
    """多步学习DQN智能体"""
    def __init__(self, state_size, action_size, n_steps=3, hidden_size=64, use_double_dqn=True):
        ...
        # 多步经验回放
        self.memory = NStepReplayBuffer(capacity=10000, n_steps=n_steps)
    ...
    def learn(self, experiences):
        ...
        # 计算目标Q值 (注意：多步回报已经包含了n步的折扣奖励，所以这里用的是gamma^n)
        targets = rewards + (1 - dones) * (self.gamma ** self.n_steps) * max_next_q
...
class StandardDQNAgent(NStepDQNAgent):
    """标准(1步)DQN智能体（用于比较）"""
    def __init__(self, state_size, action_size, hidden_size=64, use_double_dqn=True):
        super().__init__(state_size, action_size, n_steps=1, hidden_size=hidden_size, use_double_dqn=use_double_dqn)
        # 替换为标准回放缓冲区
...
def train_agent(agent, env_name='CartPole-v1', n_episodes=500, max_t=1000, 
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        if i_episode % print_every == 0:
            ...
            # 确保绘图时字体已设置
            plot_scores(scores, avg_scores, title=f"{env_name} - N-Step DQN (N={agent.n_steps})学习曲线")
...
def compare_n_step_learning(env_name='CartPole-v1', n_episodes=300):
    """比较不同n步设置的性能"""
    env = gym.make(env_name)
    ...
    # 创建智能体
    standard_agent = StandardDQNAgent(state_size, action_size, use_double_dqn=True)  # 1步
    n3_agent = NStepDQNAgent(state_size, action_size, n_steps=3, use_double_dqn=True)  # 3步
    n5_agent = NStepDQNAgent(state_size, action_size, n_steps=5, use_double_dqn=True)  # 5步
    ...
    # 训练3步DQN
    print("\n训练3步DQN...")
    n3_scores, n3_avg_scores = train_agent(
        n3_agent, env_name, n_episodes=n_episodes, print_every=100)
        
    # 训练5步DQN
    print("\n训练5步DQN...")
    n5_scores, n5_avg_scores = train_agent(
        n5_agent, env_name, n_episodes=n_episodes, print_every=100)
    ...
    plt.xlabel('回合数')
    plt.ylabel('分数')
    plt.legend(['1步DQN', '3步DQN', '5步DQN'])
    
    plt.subplot(1, 2, 2)
    plt.plot(standard_avg_scores)
    plt.plot(n3_avg_scores)
    plt.plot(n5_avg_scores)
    ...
    plt.tight_layout()
    plt.savefig(f'./n_step_comparison_{env_name}.png')
    plt.show()
    
    return standard_scores, standard_avg_scores, n3_scores, n3_avg_scores, n5_scores, n5_avg_scores
...
if __name__ == "__main__":
    ...
    plt.plot(scores)
    plt.plot(avg_scores)
    plt.title(f'{ENV_NAME} - 多步DQN (n={agent.n_steps}) 学习曲线')
    plt.xlabel('回合数')
    plt.ylabel('分数')
    plt.legend(['回合分数', '平均分数 (最近100回合)'])
    plt.savefig(f'./n_step_dqn_{ENV_NAME}_learning_curve.png')
    plt.show()
    
    # # 进行性能比较实验
    # print("\n开始比较不同n步设置的性能...")
    # compare_n_step_learning(ENV_NAME, n_episodes=300)
