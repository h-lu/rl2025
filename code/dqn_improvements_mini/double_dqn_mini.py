...
class DoubleDQNAgent:
    ...
    def learn(self, experiences):
        """从经验中更新值函数 - 使用Double DQN算法"""
        states, actions, rewards, next_states, dones = experiences
        
        # Double DQN的核心区别:
        # 1. 使用主网络选择下一个状态的最佳动作: a* = argmax(Q_main(next_states))
        # 2. 使用目标网络评估这些动作: Q_target(next_states, a*)
...
def compare_dqn_vs_double_dqn(env_name='Blackjack-v1', n_episodes=20000, eval_freq=1000):
    """比较标准DQN和Double DQN的性能"""
    # 导入标准DQN
    from dqn_mini import DQNAgent
    ...
    # 训练标准DQN
    print("\n开始训练标准DQN智能体...")
    dqn_scores, dqn_avg_scores, dqn_winrates = train_agent(dqn_agent, env_name, n_episodes, eval_freq)
    
    print("\n开始训练Double DQN智能体...")
    double_dqn_scores, double_dqn_avg_scores, double_dqn_winrates = train_agent(double_dqn_agent, env_name, n_episodes, eval_freq)
    ...
    # 平滑处理的分数
    plt.plot(dqn_avg_scores, label='标准DQN平均分数')
    plt.plot(double_dqn_avg_scores, label='Double DQN平均分数')
    
    plt.xlabel('回合')
    plt.ylabel('平均分数')
    plt.title('标准DQN vs. Double DQN 学习曲线比较')
    plt.legend()
    plt.tight_layout()
    plt.savefig("dqn_vs_double_dqn_learning_curve.png")
    plt.show()
    
    # 绘制比较获胜率图
    plt.figure(figsize=(12, 6))
    
    x_eval = [(i+1)*eval_freq for i in range(len(dqn_winrates))]
    
    plt.plot(x_eval, dqn_winrates, label='标准DQN获胜率', marker='o')
    plt.plot(x_eval, double_dqn_winrates, label='Double DQN获胜率', marker='s')
    
    plt.xlabel('训练回合')
    plt.ylabel('获胜率')
    plt.title('标准DQN vs. Double DQN 获胜率比较')
    plt.legend()
    plt.tight_layout()
    plt.savefig("dqn_vs_double_dqn_winrate.png")
    plt.show()
    
    # 最终评估
    print("\n最终评估标准DQN:")
    dqn_eval = evaluate_agent(dqn_agent, env_name)
    
    print("\n最终评估Double DQN:")
    double_dqn_eval = evaluate_agent(double_dqn_agent, env_name)
    
    print(f"\n标准DQN最终获胜率: {dqn_eval:.3f}")
    print(f"Double DQN最终获胜率: {double_dqn_eval:.3f}")
    
    # 保存模型
    dqn_agent.save("dqn_blackjack_model.h5")
    double_dqn_agent.save("double_dqn_blackjack_model.h5")
    
    return (dqn_scores, dqn_avg_scores, dqn_winrates, 
            double_dqn_scores, double_dqn_avg_scores, double_dqn_winrates)
...
if __name__ == "__main__":
    ...
    # 使用对比模式
    compare_dqn_vs_double_dqn(env_name)
    ...
    plot_scores(scores, avg_scores, 
                "Double DQN在Blackjack环境中的学习曲线", 
                "double_dqn_blackjack_learning_curve.png")
    
    # 绘制并保存获胜率曲线
    plot_winrate(winrates, 
                "Double DQN在Blackjack环境中的获胜率", 
                "double_dqn_blackjack_winrate.png")
    ...
    # 保存模型
    agent.save("double_dqn_blackjack_model.h5")
    """ 