"""
Dueling DQN 实现（微型版 - Blackjack环境）

Dueling DQN是DQN的一个架构改进，通过将Q函数分解为状态价值函数V(s)和动作优势函数A(s,a)。
这种设计可以更好地学习状态价值，提高学习的效率和稳定性。

Q(s,a) = V(s) + (A(s,a) - 1/|A| * sum(A(s,a')))

参考文献: https://arxiv.org/abs/1511.06581
"""
...
def create_standard_q_network(input_dim, action_size):
    """创建一个标准Q网络"""
...
def create_dueling_q_network(input_dim, action_size):
    """创建双路Q网络，一路估计状态价值，一路估计动作优势"""
    # 输入层
    ...
    # 状态价值流
    value_stream = layers.Dense(8, activation='relu')(x)
    value = layers.Dense(1)(value_stream)
    
    # 动作优势流
    advantage_stream = layers.Dense(8, activation='relu')(x)
    advantage = layers.Dense(action_size)(advantage_stream)
    
    # 结合状态价值和动作优势
    # 使用Keras的Lambda层包装操作，而不是直接使用tf.tile
    # 扩展value为与advantage相同的形状
    value_expanded = layers.Lambda(lambda v: keras.backend.repeat_elements(v, action_size, axis=1))(value)
    
    # 从优势中减去平均优势
    mean_advantage = layers.Lambda(lambda a: keras.backend.mean(a, axis=1, keepdims=True))(advantage)
    advantage_centered = layers.Lambda(lambda a: a[0] - a[1])([advantage, mean_advantage])
    
    # 最终Q值 = 状态价值 + (动作优势 - 平均优势)
    q_values = layers.Add()([value_expanded, advantage_centered])
...
class DuelingDQNAgent:
    ...
    def __init__(self, state_size, action_size, use_double_dqn=True):
        ...
        self.use_double_dqn = use_double_dqn  # 是否使用Double DQN的目标计算
        
        # Q网络 - 主网络和目标网络 (使用Dueling架构)
...
def compare_standard_vs_dueling_dqn(env_name='Blackjack-v1', n_episodes=20000, eval_freq=1000):
    """比较标准DQN和Dueling DQN的性能"""
    # 创建环境
    env = gym.make(env_name)
    ...
    dueling_agent = DuelingDQNAgent(state_size, action_size, use_double_dqn=True)
    ...
    print("\n开始训练Dueling DQN智能体...")
    dueling_scores, dueling_avg_scores, dueling_winrates = train_agent(dueling_agent, env_name, n_episodes, eval_freq)
    ...
    plt.plot(dueling_avg_scores, label='Dueling DQN平均分数')
    
    plt.xlabel('回合')
    plt.ylabel('平均分数')
    plt.title('标准DQN vs. Dueling DQN 学习曲线比较')
    plt.legend()
    plt.tight_layout()
    plt.savefig("standard_vs_dueling_dqn_learning_curve.png")
    plt.show()
    ...
    plt.plot(x_eval, dueling_winrates, label='Dueling DQN获胜率', marker='s')
    
    plt.xlabel('训练回合')
    plt.ylabel('获胜率')
    plt.title('标准DQN vs. Dueling DQN 获胜率比较')
    plt.legend()
    plt.tight_layout()
    plt.savefig("standard_vs_dueling_dqn_winrate.png")
    plt.show()
    ...
    print("\n最终评估Dueling DQN:")
    dueling_eval = evaluate_agent(dueling_agent, env_name)
    
    print(f"\n标准DQN最终获胜率: {standard_eval:.3f}")
    print(f"Dueling DQN最终获胜率: {dueling_eval:.3f}")
    
    # 保存模型
    standard_agent.save("standard_dqn_blackjack_model.h5")
    dueling_agent.save("dueling_dqn_blackjack_model.h5")
    
    return (standard_scores, standard_avg_scores, standard_winrates,
            dueling_scores, dueling_avg_scores, dueling_winrates)
...
if __name__ == "__main__":
    ...
    # 使用对比模式
    compare_standard_vs_dueling_dqn(env_name)
    ...
    # 训练智能体
    print("开始训练Dueling DQN智能体...")
    scores, avg_scores, winrates = train_agent(agent, env_name)
    
    # 绘制并保存学习曲线
    plot_scores(scores, avg_scores, 
                "Dueling DQN在Blackjack环境中的学习曲线", 
                "dueling_dqn_blackjack_learning_curve.png")
    
    # 绘制并保存获胜率曲线
    plot_winrate(winrates, 
                "Dueling DQN在Blackjack环境中的获胜率", 
                "dueling_dqn_blackjack_winrate.png")
    ...
    # 保存模型
    agent.save("dueling_dqn_blackjack_model.h5")
    """ 